"""Config flow for the Learning Sensor integration."""
...
async def _validate_subentry_from_config_entry(
    ...
    if handler.parent_handler.cur_step is not None:
        assert "step_id" in handler.parent_handler.cur_step, (
            "Parent step must have a step_id"
        )
...
class LearningSensorConfigFlowHandler(SchemaConfigFlowHandler, domain=DOMAIN):
